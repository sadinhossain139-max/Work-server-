...
async def worker_health(api_key: str = Depends(verify_api_key)):
    ...
    return {
        ...
        "load_percentage": (len(telegram_manager.active_clients) / config.MAX_CONCURRENT_SESSIONS) * 100 if config.MAX_CONCURRENT_SESSIONS > 0 else 0,
        ...
        "processing_tasks": len(telegram_manager.processing_tasks) if hasattr(telegram_manager, 'processing_tasks') else 0
    }
